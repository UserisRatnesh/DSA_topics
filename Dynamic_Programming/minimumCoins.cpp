...
int helper_memo(int index, int amount, vector<int> &coins,
  ...
  int take = 1e9;
  if (amount >= coins[index]) {
    take = 1 + helper_memo(index, amount - coins[index], coins, dp);
  }
...
int memo(vector<int> &coins, int amount) {
  ...
  int ans = helper_memo(0, amount, coins, dp);
  if (ans == 1e9) {
    return -1;
  }
  return ans;
  ...
  for (int i = n - 1; i >= 0; i--) {
    for (int j = 0; j <= amount; j++) {
      ...
      if (j >= coins[i]) {
        take =
            1 +
            dp[i][j - coins[i]]; // It is working because
                                 // dp[i][j-cons[i]] is already solved
                                 // before hand, You can check by doing dry run
      }
